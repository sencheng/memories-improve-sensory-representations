...
class EpisodicMemory():
    def __init__(self, p_dim, retrieval_length, retrieval_noise, category_weight=0,
        ...
        if self.depression:
            ...
            self.depression_afun = eval(depress_params.get('activation_function',
                                                           'lambda X: X'))  # CARE: afun is not used in current implementation
    ...
    def retrieve_sequence(self, cue_or_idx, cue_category=None, ret_len=None, ret_noise=None, return_jumps=False,
        ...
        if self.changed:  # if list was changed,
            ...
            if len(self.memory_list) == 1:
                np.append(self.memory_list[0], self.memory_list[0][
                    -1])  # if only one sequence was stored, we want to detect if we reach the end of the sequence, so we need to include last element
                if self.use_helper_array:
            ...
            if self.use_categories:
                ...
                if len(self.memory_list) == 1:
                    np.append(self.category_list[0], self.category_list[0][
                        -1])  # only one sequence, repeat last element, as for sequences above
                self.category_arr = self.category_weight * np.concatenate([cat_list[:-1] for cat_list in
                                                                           self.category_list])  # as it happens for sequences, remove last element from cat array
        ...
        while True:
            ...
            if last_index == -1:
                ...
                # last_index ist used later once:
                # if several candidates with minimum distance are found, select the closest, no problem because cue anyway was not in memory as key
            ...
            if len(min_dist) > 1:  # if multiple indiforming_categoriesces with minimum distance
