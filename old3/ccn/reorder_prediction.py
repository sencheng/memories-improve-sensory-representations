...
in order to make figure 13 using :py:mod:`analReorderNoise`.
...
if __name__=="__main__":
    ...
    xlbls = [2,3,4,5,6,7,8,9,10,12,14,16,18,20,24,28,32,36,40,45,50,60,70,80,90,100]
    ...
    # # ax[0].legend()
    plt.plot(xlbls[0], xymean_s[0], c="k", mfc="none", marker="^", markersize=MS, mew=MEW, clip_on=False)
    plt.plot(xlbls[0], xymean_e[0], c="k", mfc="none", marker="v", markersize=MS, mew=MEW, clip_on=False)
    ...
    lc1r, = plt.plot(xlbls, rcatmean_s[:len(xlbls)], c=ccolors[1], ls=cstyles[1])
    lc2r, = plt.plot(xlbls, rcatmean_e[:len(xlbls)], c=ccolors[3], ls=cstyles[3])
    ...
    l1, = plt.plot(xlbls, xymean_s[:len(xlbls)], label="simple", c=ccolors[0], ls=cstyles[0])
    l2, = plt.plot(xlbls, xymean_e[:len(xlbls)], label="episodic", c=ccolors[2], ls=cstyles[2])
    ...
    plt.xlabel("length of training episodes")
    ...
    plt.legend((l2, lc2r,l1,l2), ("x,y", "identity", "simple", "episodic"), loc=4, ncol=2)
