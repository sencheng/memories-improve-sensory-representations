...
try:
    ...
    for i, let in enumerate(letters):
        ...
        for di, d in enumerate(depr):
            ...
            phiZ_E[i].append([])
            d_form[i].append([])
            d_retrieved[i].append([])
            for ri in range(31):
                rind = ri*len(depr)+di
                print(rind)
                fname = "res{}.p".format(rind)
                ...
                d_s[i][di].append(np.mean(np.sort(res.d_values['testingZ_S'])[1]))
                d_e[i][di].append(np.mean(np.sort(res.d_values['testingZ_E'])[1]))

                d_lat_s[i][di].append(np.mean(res.d_values['forming_lat'][:2]))
                d_cat_s[i][di].append(np.mean(res.d_values['forming_cat']))
                d_lat_e[i][di].append(np.mean(res.d_values['retrieved_lat'][:2]))
                d_cat_e[i][di].append(np.mean(res.d_values['retrieved_cat']))

                d_form[i][di].append(np.mean(np.sort(res.d_values['forming_Y'])[:16]))
                d_retrieved[i][di].append(np.mean(np.sort(res.d_values['retrieved_Y'])[:16]))

                cat_s[i][di].append(np.max(np.abs(res.testing_corrZ_simple[4, :])))
                cat_e[i][di].append(np.max(np.abs(res.testing_corrZ_episodic[4, :])))
                ...
                xy_s[i][di].append(np.mean((r_valueX_S, r_valueY_S)))
                rcat_s[i][di].append(r_valueCAT_S)
                phiZ_S[i][di].append(np.mean((r_valueCosphi_S, r_valueSinphi_S)))
                xy_e[i][di].append(np.mean((r_valueX_E, r_valueY_E)))
                rcat_e[i][di].append(r_valueCAT_E)
                phiZ_E[i][di].append(np.mean((r_valueCosphi_E, r_valueSinphi_E)))
...
ax[0].plot(xlbls, dmean_s[0], label="simple", c='g', ls='-')
lc1r, = ax[1].plot(xlbls, rcatmean_s[0], c='g', ls='--')
l1, = ax[1].plot(xlbls, xymean_s[0], label="simple", c='g', ls='-')
lp1, = ax[1].plot(xlbls, phimean_s[0], label="simple", c='g', ls=':')
ldl, = ax[2].plot(xlbls, d_latmean_s[0], c='g', ls='-')
ldc, = ax[2].plot(xlbls, d_catmean_s[0], c='g', ls='--')
ldf, = ax[2].plot(xlbls, d_formmean[0], c='g', ls=':')
lc2r, l2, lp2 = [], [], []
for di, d in enumerate(depr):
    ax[0].plot(xlbls, dmean_e[di], label="episodic {}".format(d), c=['r', 'm', 'b', 'c', 'k'][di], ls='-')

    tmp, = ax[1].plot(xlbls, rcatmean_e[di], c=['r', 'm', 'b', 'c', 'k'][di], ls='--')
    lc2r.append(tmp)
    tmp, = ax[1].plot(xlbls, xymean_e[di], label="episodic {}".format(d), c=['r', 'm', 'b', 'c', 'k'][di], ls='-')
    l2.append(tmp)
    tmp, = ax[1].plot(xlbls, phimean_e[di], label="episodic {}".format(d), c=['r', 'm', 'b', 'c', 'k'][di], ls=':')
    lp2.append(tmp)

    ax[2].plot(xlbls, d_latmean_e[di], c=['r', 'm', 'b', 'c', 'k'][di], ls='-')
    ax[2].plot(xlbls, d_catmean_e[di], c=['r', 'm', 'b', 'c', 'k'][di], ls='--')

    ax[2].plot(xlbls, d_retrievedmean[di], c=['r', 'm', 'b', 'c', 'k'][di], ls = ':')
...
lins.extend(l2)
labs = ["simple"]
labs.extend(["episodic{}".format(dep) for dep in depr])
# plt.figlegend((l1,l2), ("simple", "episodic"), 1)
plt.figlegend(lins, labs, 1)
plt.show()
